...
- [x] **GYM-04 — Make offline/syncing state visible to users** (ready)
...
- [x] **GYM-05 — Define the jargon: rolling, split, catch-up, makeup day, RPE** (ready)
...
- [x] **GYM-06 — Bring logging into Home for new users** (ready)
...
- **2026-09-19 — two dispatches were interrupted by WebUI restarts.** The GYM-03+04+06 batch started at 21:37:44 and the serial GYM-03 retry started at 21:41:23. Their owner process was deliberately restarted at 21:41:02 and 21:45:32 during WebUI supervisor/auth setup, so Hermes correctly reported "Delegation owner exited before recording a terminal result." This was not a dispatcher failure: GYM-06 completed an `execute_code` call before the first restart, and the serial GYM-03 worker read three files before the second. Separately, SSHFS 3.7.6 enables `contain_symlinks` by default, which rejects the absolute target used by `api/.venv/bin/python` and the `..` target used by `frontend/node_modules/.bin/*`. An isolated mount verified that `no_contain_symlinks` restores both forms; that option is staged in the persistent mount unit and takes effect after the next idle remount. Until then, use a local temporary venv and invoke frontend modules directly (`node node_modules/vitest/vitest.mjs`, `node node_modules/typescript/bin/tsc`). Card status: **GYM-03 `f2aad77`; GYM-04 `eeab25e`; GYM-06 `6a075d5`; GYM-05 `a2d4952` — all done.**
