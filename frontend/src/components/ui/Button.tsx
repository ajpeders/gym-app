...
const container: Record<Variant, string> = {
  ...
  secondary: 'border border-iron-700 bg-iron-900/40 active:bg-iron-800',
...
const sizing: Record<Size, string> = {
  sm: 'min-h-[38px] px-3.5 py-2 rounded-md',
  md: 'min-h-[48px] px-4 py-3 rounded-md',
  lg: 'min-h-[54px] px-5 py-3.5 rounded-md',
};
...
export function Button({
  ...
  const iconColor = variant === 'primary' ? '#05080f' : variant === 'danger' ? '#ffffff' : '#38bdf8';
  ...
  return (
    <Pressable
      ...
      {...rest}>
      {loading ? (
        <ActivityIndicator color={variant === 'secondary' || variant === 'ghost' ? '#38bdf8' : '#05080f'} />
      ) : (
